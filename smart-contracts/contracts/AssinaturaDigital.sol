...
abstract contract AssinaturaDigital is EIP712, Nonces {
    ...
    function _assinar(address assinante,uint16 numeroDoCandidato,uint256 prazo ,uint8 v,bytes32 r,bytes32 s) internal virtual{
        if(block.timestamp > prazo) revert AssinaturaDigital__ForaDoPrazo();
        bytes32 structHash = keccak256(abi.encode(VOTACAO_TYPEHASH,assinante,numeroDoCandidato,_useNonce(assinante),prazo));
